...
def train_clas(dir_path, cuda_id, lm_id='', clas_id=None, bs=64, cl=1, backwards=False, startat=0, unfreeze=True,
    ...
    if useWeightSampler:
        wl2i = []
        l2p = [0.88, 0.04, 0.04, 0.04]
        #l2p = [0.1, 0.8, 0.8, 0.8]
        #l2p = [0.88, 0.1, 0.1, 0.1]
        for ind, d in enumerate(tqdm(trn_ds)):
            wl2i.append(l2p[d[1]])
        trn_samp = WeightedRandomSampler(wl2i, len(trn_ds), replacement=True)
        #trn_samp = WeightedRandomSampler(wl2i, len(trn_ds), replacement=False)
        pass
    else:
    ...
    if use_att:
       ...
       #                           dropouti=dps[0], wdrop=dps[1], dropoute=dps[2], dropouth=dps[3])
        m = get_rnn_classifier_w3(bptt, 20 * 70, c, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
                              layers=[em_sz*2, 100, c], drops=[dps[4], 0.1],
                               dropouti=dps[0], wdrop=dps[1], dropoute=dps[2], dropouth=dps[3])
    else:
