...
def generateTestOut(testfilePath, classPath, LMPath, classModelName, LMModelName, targetTestOut):
    ...
    prediction, samplIdx = eval_clas_eRisk(classPath, 0, lm_id=LMModelName, clas_id=classModelName, attention=True, bs=64, backwards=False)

    idA = [idA[i] for i in samplIdx]
    t1A = [t1A[i] for i in samplIdx]
    t2A = [t2A[i] for i in samplIdx]
    t3A = [t3A[i] for i in samplIdx]
    ...
    with open(targetTestOut, 'w', encoding='utf8') as outF:
        ...
        for id,t1,t2,t3,labN in zip(idA, t1A, t2A, t3A, prediction):
