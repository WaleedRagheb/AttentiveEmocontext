...
#classModelNames = ['ph1Class_recap_BEST','ph1Class_recap_BEST','ph1Class_ws_1','ph1Class_ws_n_BEST']
#classModelNames = ['ph1Class_recap_BEST','ph1Class_recap_BEST','ph1Class_ws_3','ph1Class_ws_n_BEST']
classModelNames = ['ph1Class_recap_BEST','ph1Class_recap_BEST','ph1Class_recap','ph1Class_recap']
...
def generateTestOut(testfilePath, classPath, LMPath, classModelNames, LMModelName, targetTestOut):
    ...
    for classModelName in classModelNames:
        ...
        prediction_i, samplIdx_fw = eval_clas_eRisk(classPath, 0, lm_id=LMModelName, clas_id=classModelName, attention=True, bs=64, backwards=False, scores=True)

        predAll.append(prediction_i)


    prediction_all = [np.argmax(((softmax(p1)) + (softmax(p2)) + (softmax(p3)) + (softmax(p4)))/4.0, axis=0) for p1,p2,p3,p4 in zip(predAll[0],predAll[1],predAll[2],predAll[3])]
