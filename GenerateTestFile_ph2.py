...
targetTestOut = "data/nlp_clas/train_ALL/train_ALL_clas/tmp/tests/phase2/test.txt"
...
def generateTestOut(testfilePath, classPath, LMPath, classModelName, LMModelName, targetTestOut):
    ...
    prediction_fw, samplIdx_fw = eval_clas_ph2(classPath, 0, lm_id=LMModelName, clas_id=classModelName,
                                                 attention=True, bs=70, backwards=False, scores=True)
    prediction_bw, samplIdx_bw = eval_clas_ph2(classPath, 0, lm_id=LMModelName, clas_id=classModelName,
                                                 attention=True, bs=70, backwards=True, scores=True)

    prediction_all = [np.argmax((softmax(p_fw) + softmax(p_bw)) / 2, axis=0) for p_fw, p_bw in
                      zip(prediction_fw, prediction_bw)]
